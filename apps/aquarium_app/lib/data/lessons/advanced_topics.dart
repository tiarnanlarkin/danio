    ...
    Lesson(
      ...
      sections: [
        ...
        const LessonSection(
          type: LessonSectionType.warning,
          content:
              'Danio is educational guidance, not a substitute for professional diagnosis. Escalate to an aquatic vet or experienced aquatic professional when severe symptoms persist, multiple fish are affected, or medication decisions are uncertain.',
        ),
        const LessonSection(
          type: LessonSectionType.bulletList,
          content:
              '• **Ammonia spike:** Do a 50% water change IMMEDIATELY. If available, use a dechlorinator labelled for emergency ammonia/nitrite support and follow the product label exactly. Stop feeding. Find the cause: dead fish hidden somewhere? Overfeeding? Filter crashed?\n• **Fish gasping at surface:** Increase surface agitation — lower the water level so the filter outflow splashes, or add an airstone. Do a water change. Check temperature (warm water holds less oxygen).\n• **Ich (white spots) — TROPICAL FISH ONLY:** Raise temperature to 86°F (30°C) gradually over 24 hours + add aquarium salt only when appropriate, using a common reference dose of 1 tbsp per 19 litres (5 gallons). Treat for 2 full weeks — the temperature breaks the ich life cycle. ⚠️ COLDWATER WARNING: Do NOT raise temperature above 24°C/75°F for coldwater fish (goldfish, white cloud minnows, hillstream loaches). High heat WILL kill them. For coldwater species, use medication-only treatment (e.g. API Super Ick Cure or similar) without heat treatment. Salt is also harmful to scaleless fish (loaches, corydoras) and planted tanks — use medication instead. See the Fish Health learning path for full guidance.\n• **Power outage:** Wrap the tank in blankets for insulation. Use a battery-powered air pump if available. Don\'t feed during the outage.',
        ),
        const LessonSection(
          type: LessonSectionType.heading,
        ...
        const LessonSection(
          type: LessonSectionType.tip,
          content:
              'Useful emergency kit: a broad water conditioner, liquid test kit, aquarium salt for species-appropriate uses, and a battery-powered air pump. These supplies support a faster response, but water changes, testing, and correct diagnosis still drive the outcome.',
        ),
      ],
      quiz: Quiz(
        id: 'at_trouble_quiz',
