...
class LeaderboardScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return profileAsync.when(
      data: (profile) {
        ...
        return _buildLeaderboard(context, entries, currentWeek, currentEntry);
      },
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          _buildHeader(context, currentWeek, currentEntry),
          
          // Promotion/demotion zones indicator
          _buildPromoInfo(context, currentEntry, entries.length),
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return _buildLeaderboardTile(context, entry, currentEntry);
              },
            ),
          ),
        ],
  ...
  Widget _buildHeader(BuildContext context, WeekPeriod week, LeaderboardEntry currentEntry) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: _leagueColors(currentEntry.league),
      ...
      child: Column(
        children: [
          ...
          Row(
            ...
            children: [
              Icon(
                _leagueIcon(currentEntry.league),
                color: Colors.white,
                size: 32,
              ),
              const SizedBox(width: 8),
              Text(
                '${currentEntry.league.displayName} League',
  ...
  Widget _buildPromoInfo(BuildContext context, LeaderboardEntry currentEntry, int totalUsers) {
    final willPromote = currentEntry.rank <= LeagueThresholds.topPromotion && 
                       currentEntry.league != League.diamond;
    final willDemote = currentEntry.rank > totalUsers - LeagueThresholds.bottomDemotion && 
                      currentEntry.league != League.bronze;
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              willPromote
                  ? 'You\'re in the promotion zone! Finish top 3 to advance to ${_nextLeague(currentEntry.league)?.displayName} League!'
                  : 'You\'re at risk of demotion. Finish higher to stay in ${currentEntry.league.displayName} League!',
              style: TextStyle(
  ...
  Widget _buildLeaderboardTile(BuildContext context, LeaderboardEntry entry, LeaderboardEntry currentEntry) {
      ...
      child: ListTile(
        ...
        trailing: Column(
          ...
          children: [
            ...
            if (isPromotionZone && entry.league != League.diamond)
              const Icon(Icons.arrow_upward, color: Colors.green, size: 16)
            else if (isDemotionZone && entry.league != League.bronze)
