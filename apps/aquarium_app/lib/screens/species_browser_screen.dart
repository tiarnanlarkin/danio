...
class _CareActionsCard extends ConsumerWidget {
  ...
  Future<void> _addToTank(BuildContext context, WidgetRef ref) async {
    final selectedTank = await _selectTank(
      context,
      ref,
      emptyMessage: 'Add a tank first before adding livestock.',
    );
  ...
  Future<void> _createCareTask(BuildContext context, WidgetRef ref) async {
    final selectedTank = await _selectTank(
      context,
      ref,
      emptyMessage: 'Add a tank first before creating care tasks.',
    );
    if (selectedTank == null || !context.mounted) return;
    ...
    try {
      ...
      final title = 'Review ${species.commonName} care plan';
      final description = _careTaskDescription();
      final task = existing == null
          ...
          : existing.copyWith(
              ...
              description: description,
              recurrence: RecurrenceType.weekly,
              dueDate: existing.dueDate ?? dueTomorrow,
              priority: TaskPriority.normal,
              isEnabled: true,
              isAutoGenerated: true,
              updatedAt: now,
      ...
      if (context.mounted) {
        DanioSnackBar.error(
          context,
          'Could not create a care task for ${species.commonName}',
        );
      }
    }
  }

  Future<Tank?> _selectTank(
    BuildContext context,
    WidgetRef ref, {
    ...
    if (tanks.isEmpty) {
      DanioSnackBar.warning(context, emptyMessage);
      return null;
    }
  ...
  String _speciesTaskId(Tank tank) {
    final tankSlug = tank.id.replaceAll(RegExp(r'[^A-Za-z0-9_-]+'), '_');
    final speciesSlug = species.scientificName
        .toLowerCase()
        .replaceAll(RegExp(r'[^a-z0-9]+'), '_')
        .replaceAll(RegExp(r'^_+|_+$'), '');
    return 'species_${tankSlug}_${speciesSlug}_care';
  }

  String _careTaskDescription() {
    final lines = <String>[
      'Minimum group: ${species.minSchoolSize}',
      'Minimum tank: ${species.minTankLitres.toStringAsFixed(0)} L',
      'Temperature: ${_format(species.minTempC)}-${_format(species.maxTempC)} C',
      'pH: ${_formatPh(species.minPh)}-${_formatPh(species.maxPh)}',
    ];

    if (species.avoidWith.isNotEmpty) {
      lines.add('Tankmates to review: ${species.avoidWith.join(', ')}');
    }

    if (species.medicationWarnings.isNotEmpty) {
      lines.add('Review treatment warnings before medicating.');
    }
