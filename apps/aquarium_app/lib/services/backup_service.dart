...
class BackupService {
  ...
  void _validateTankScopedCollection(
    ...
    for (final entry in entries) {
      ...
      for (final field in _optionalBooleanChildFields(collectionName)) {
        final value = entry[field];
        if (value != null && value is! bool) {
          throw Exception(
            'Invalid format: $collectionName $field values must be booleans',
          );
        }
      }
      for (final field in _optionalObjectChildFields(collectionName)) {
        final value = entry[field];
        if (value != null && value is! Map) {
          throw Exception(
            'Invalid format: $collectionName $field values must be objects',
          );
        }
      }
  ...
  List<String> _optionalBooleanChildFields(String collectionName) {
    return switch (collectionName) {
      'tasks' => const ['isEnabled', 'isAutoGenerated'],
      _ => const [],
    };
  }

  List<String> _optionalObjectChildFields(String collectionName) {
    return switch (collectionName) {
      'equipment' => const ['settings'],
      _ => const [],
    };
  }
