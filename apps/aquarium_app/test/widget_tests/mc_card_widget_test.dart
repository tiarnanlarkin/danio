...
void main() {
  group('McCardWidget -- rendering', () {
    ...
    testWidgets('disables answer-tile transitions for reduced motion', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrap(
      ...
      final optionAnimations = tester.widgetList<AnimatedContainer>(
        find.byType(AnimatedContainer),
      );

      expect(optionAnimations, isNotEmpty);
      expect(
        optionAnimations.every(
          (animation) => animation.duration == Duration.zero,
        ),
        isTrue,
      );
    });
  ...
  group('McCardWidget -- wrong answer', () {
    ...
    testWidgets('onAnswered fires with false for wrong answer', (tester) async {
      bool? answeredCorrectly;
      final question = _makeQuestion(correctIndex: 2);
