...
void main() {
  ...
  group('CostTrackerScreen - with saved data', () {
    ...
    testWidgets(
      'single expense delete failure keeps expense visible with feedback',
      (tester) async {
        const savedExpenses =
            '[{"id":"1","description":"Filter","amount":35.0,'
            '"category":"Equipment","date":"2025-01-15T12:00:00.000"},'
        ...
        expect(find.text('Plant food'), findsOneWidget);
        expect(
          find.text("Couldn't delete that expense. Try again in a moment."),
          findsOneWidget,
        );
        ...
        final saved =
            jsonDecode(prefs.getString('cost_tracker_expenses')!)
                as List<dynamic>;
        expect(saved.map((e) => e['description']), ['Filter', 'Plant food']);
      },
    );
