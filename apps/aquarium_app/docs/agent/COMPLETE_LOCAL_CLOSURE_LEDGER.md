...
| DCL-DR-003 | Broader create/edit/delete/undo data-resilience coverage needs final P0/P1 closure. | F1 through F34 are settled. Fresh 2026-07-19 read-only inspection identified Tasks completion, Equipment Mark Serviced, and single livestock-add compensation boundaries plus a Wishlist replay probe in the fixed release sequence. | `DCL_DR_003_CRUD_UNDO_RESILIENCE_MATRIX.md`; `FINISH_MAP.md`; rolling and archived `SLICE_LOG.md`. | `VERIFY_LOCALLY` | open | Data resilience | No | Complete the three planned compensation epochs and Wishlist replay probe with focused RED/GREEN discipline, then reinspect the current matrix. Resolve every proven P0/P1, explicitly park lower severity boundaries, pass Full, and do not infer closure from F34 alone. |
...
`DCL-DR-001` and `DCL-DR-002` are closed. `DCL-DR-003` remains open after
settled findings F1 through F34. After authority epoch `DR-2026-07-19-057` is
clean, pushed, and aligned, the next manual product epoch is Tasks completion
compensation from
`plans/2026-07-19-phone-release-candidate-finalization-plan.md`. Prove
simultaneous completion-log and task-restore failure, preserve both causes,
refresh task/tank/log authority, block stale callbacks, warn honestly, and omit
unsafe Retry. Equipment Mark Serviced and single livestock-add follow as
separate epochs; then run the Wishlist replay probe. Do not select
`DCL-DR-004` or a later sequence item first.
