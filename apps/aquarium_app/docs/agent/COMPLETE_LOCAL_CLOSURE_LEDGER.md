...
unstarted. Begin only in a new explicit manual epoch after E0 is clean, pushed,
and aligned, and perform the ordered read-only restore-matrix audit before any
implementation. Select no other product row unless current evidence or a user
decision explicitly changes the target.
