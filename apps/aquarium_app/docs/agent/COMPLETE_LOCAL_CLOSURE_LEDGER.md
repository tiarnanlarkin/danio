...
| DCL-MOTION-001 | Global haptic-preference enforcement is complete; five phone-quality clusters still need reduced-motion and quality evidence. | All product haptics route through one persisted preference-aware adapter; the source guard permits platform haptics only inside it; disabled emits zero platform calls and enabled actions use exact non-duplicated sequences. | `DR-2026-07-22-068`; `FINISH_MAP.md` Motion and haptics row; current release-candidate plan; haptic boundary/adapter/feedback/action tests and reduced-motion checks. | `VERIFY_LOCALLY` | open | Phone motion and visual polish | No | The global haptic adapter, source guard, zero-call disabled proof, and exact enabled-call proof pass; reduced-motion and the remaining quality checks pass across all five phone-quality clusters. |
