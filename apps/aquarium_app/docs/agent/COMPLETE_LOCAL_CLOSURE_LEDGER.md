...
| DCL-DR-003 | Broader create/edit/delete/undo data-resilience coverage needs final P0/P1 closure. | F1 through F35 are settled. Tasks completion compensation is fixed in `DR-2026-07-19-058`; Equipment Mark Serviced compensation next, then single livestock-add and the Wishlist replay probe remain in fixed order. | `DCL_DR_003_CRUD_UNDO_RESILIENCE_MATRIX.md`; `FINISH_MAP.md`; rolling and archived `SLICE_LOG.md`. | `VERIFY_LOCALLY` | open | Data resilience | No | Complete the two remaining planned compensation epochs and Wishlist replay probe with focused RED/GREEN discipline, then reinspect the current matrix. Resolve every proven P0/P1, explicitly park lower severity boundaries, pass Full, and do not infer closure from F35 alone. |
...
settled findings F1 through F35. Tasks completion compensation is fixed in
`DR-2026-07-19-058`. Run Equipment Mark Serviced compensation next, then
single livestock-add as a separate epoch, then the Wishlist replay probe. Do
not select `DCL-DR-004` or a later sequence item first.
