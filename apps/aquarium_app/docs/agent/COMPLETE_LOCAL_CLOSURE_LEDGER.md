...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| DCL-DR-001 | Restore walkthrough and restore failure behavior still need closure evidence beyond the recent photo/import hardening. | The phone completion program begins product work with Phase 1 data resilience; the Finish Map and backlog keep Backup/Data resilience in progress after DS-043. | Phase 1 Task 1.1; `DR-2026-07-16-001` through `DR-2026-07-16-004`; `DCL_DR_001_RESTORE_BEHAVIOR_MATRIX.md`; F1 preserves preference errors, F2 makes export-share status honest and cleans completed ZIPs, F3 proves file-selection outcomes, and F4 proves valid-preview cancellation has no restore side effects. `DCL-DR-001-F5` is next because simultaneous tank-import/rollback failure remains source-explained but unproven. | `VERIFY_LOCALLY` | open | Data resilience | No | Prove `DCL-DR-001-F5` first, then add direct mid-extraction-cleanup evidence; fix only proven gaps, leave no matrix path unexplained, and pass one final Full gate. |
...
`DCL-DR-001-F5` is the next unproven ordered path. Begin its exact-marker
simultaneous tank-import/rollback-failure proof only after this checkpoint is
