...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| DCL-DR-002 | Schema migration and corruption-recovery paths remain open for complete-local confidence. | `DR-2026-07-16-007` found and fixed the inaccessible `ioError` retry; independent review then recorded the separate false recovery-copy assurance as F2. | `DCL_DR_002_MIGRATION_CORRUPTION_RECOVERY_MATRIX.md`; `I/O load error offers real retry without destructive start fresh`; `FINISH_MAP.md` Backup and restore row. | `VERIFY_LOCALLY` | open | Data resilience | No | Migration/corruption paths are proven by source/test or owned-device walkthrough evidence; any missing behavior is fixed locally and Full gate passes. |
...
open; `DCL-DR-002-F1` is locally fixed. After this checkpoint is clean, pushed,
and aligned, continue the next manual F2 recovery-copy honesty fix under marker
`danio-dcl-dr-002-recovery-copy-honesty-2026-07-16/1`. Do not select
