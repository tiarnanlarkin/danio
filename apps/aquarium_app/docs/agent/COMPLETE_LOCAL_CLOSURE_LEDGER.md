...
| --- | --- | --- | --- | --- | --- | --- | --- |
| DCL-DR-001 | Restore walkthrough and restore failure behavior still need closure evidence beyond the recent photo/import hardening. | Finish Map and backlog both keep Backup/Data resilience in progress after DS-043. | `FINISH_MAP.md` Backup and restore row; `FINISH_MAP.md` ranked roadmap rank 1; backlog `CL-P1-009`; `CL-QA-006`. | `VERIFY_LOCALLY` | Data resilience | No | Current restore paths are audited from source/tests, any missing false-success or rollback behavior is fixed with RED/GREEN proof, and Full gate passes. |
...
| DCL-DR-005 | Future debounced local writers need app-kill flush coverage when new debounced writers are added. | Finish Map explicitly calls out future debounced-writer app-kill recheck. | `FINISH_MAP.md` Data resilience row; accelerated plan Epoch 3. | `VERIFY_LOCALLY` | Data resilience | No | Current debounce/timer writer inventory is compared with lifecycle tests; missing flush behavior is fixed or row is marked no-current-gap with evidence. |
