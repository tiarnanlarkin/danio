...
Historical Cluster 3 closeout marker: two phone-quality clusters remain.
...
- Android Keystore-backed secure-storage migration is closed under
...
| DCL-DR-004 | Livestock-removal backup tombstone relationships have complete-local proof. | `DR-2026-07-21-063` proved valid `livestockRemoved` history can outlive its livestock row and fixed preview/import validation plus remapping without weakening other relationships. | `preserves a deleted livestock tombstone through self-generated preview and import`; both preview-normalized live-target tests; independent settled-diff review; reset-assisted Full signing/dependency/lint/tests/analyze/APK in 213,027 ms. | `VERIFY_LOCALLY` | closed | Data resilience | No | Reopen only if backup relationship validation/remapping changes or contradictory evidence shows a tombstone can enter a live map, resurrect livestock, or weaken another invalid-relationship rejection. |
| DCL-AI-001 | Any local persistence of AI output requires explicit user confirmation. | Fish ID and AI Compatibility each show their result before consent, write nothing on dismiss/cancel, write once on confirmation, and keep the result visible after history-save failure. Symptom Triage, Weekly Plan, Ask Danio, and Fish ID consent/history regressions were reverified after a source audit found exactly those four callers plus Compatibility. | `lib/features/smart/fish_id/fish_id_screen.dart`; `test/widget_tests/fish_id_screen_test.dart`; `lib/widgets/compatibility_checker_widget.dart`; `test/widget_tests/compatibility_checker_widget_test.dart`; Compatibility Focused PASS in 10,554 ms; nine prior-surface tests PASS; independent review; reset-assisted Full signing/dependency/lint/full-tests/analyze/APK PASS in 183,065 ms. | `VERIFY_LOCALLY` | closed | Optional AI | No | Fish ID and AI Compatibility each prove dismiss/cancel writes nothing, confirm writes exactly once, and a history-write failure never hides the visible result; every already-gated AI surface is reverified. |
| DCL-P1-003 | Guided tools have settled current save/apply evidence. | `DR-2026-07-22-067` reverified the accepted Workshop paths and all five Tank Volume shapes without finding a concrete save/apply defect. | Workshop and Tank Volume tests; 152-test Focused/analyze PASS in 20,704 ms; independent review; reset-assisted Full signing/dependency/lint/tests/analyze/APK PASS in 233,189 ms. | `VERIFY_LOCALLY` | closed | Normal-user P1 depth | No | Reopen only if a guided-tool save/apply path changes or contradictory current evidence proves a concrete gap. |
| DCL-P1-004 | Timeline and journal have settled current handoff proof. | `DR-2026-07-22-067` reverified the complete Journal suite and current persistence/label/context paths without finding a concrete defect. | `test/widget_tests/journal_screen_test.dart`; 152-test Focused/analyze PASS in 20,704 ms; independent review; reset-assisted Full PASS in 233,189 ms. | `VERIFY_LOCALLY` | closed | Normal-user P1 depth | No | Reopen only if timeline/journal persistence, labels, or context handoffs change or contradictory current evidence proves a gap. |
| DCL-P1-005 | Learning has settled validator and representative-path proof at accepted breadth. | The accepted 82-lesson validators and representative lesson/path tests passed in `DR-2026-07-22-067` without requiring breadth expansion. | Lesson-data, content-validator, and lazy-path-card tests; 152-test Focused/analyze PASS in 20,704 ms; reset-assisted Full PASS in 233,189 ms. | `VERIFY_LOCALLY` | closed | Normal-user P1 depth | No | Reopen only if learning content/path behavior changes or contradictory current evidence proves a defect. |
| DCL-P1-006 | Species and plants have settled validator and representative-path proof at accepted breadth. | Current species/plant browser and validator suites passed in `DR-2026-07-22-067` without requiring database expansion. | Species/plant browser and content-validator tests; 152-test Focused/analyze PASS in 20,704 ms; reset-assisted Full PASS in 233,189 ms. | `VERIFY_LOCALLY` | closed | Content and P1 depth | No | Reopen only if species/plant content or representative paths change or contradictory current evidence proves a defect. |
| DCL-PREF-001 | Preferences has secure user AI-key storage and final provider/privacy proof. | `DR-2026-07-21-066` routes Optional-AI credentials through `ApiKeyStore` and Android Keystore-backed secure storage. Successful migration removes the legacy preference only after secure persistence; failure retains it; deletion attempts secure and legacy locations; Android reset-on-error is disabled; failure copy remains honest and keyless local behavior is preserved. | `lib/services/api_key_store.dart`; service, Settings, deletion, debug, backup, privacy, and provider tests; 128-test Focused/analyze PASS in 22,291 ms; no-added-plaintext source audit; independent review; reset-assisted Full signing/dependency/lint/2,303 tests/analyze/APK PASS in 243,873 ms. | `VERIFY_LOCALLY` | closed | Preferences and Optional AI | No | Reopen only if credential storage, migration/deletion, backup exclusion, keyless/provider/privacy behavior, or Android secure-storage configuration changes, or contradictory evidence shows plaintext or silent key loss. |
| DCL-CONTENT-001 | Current content and source-risk validators have settled proof. | `DR-2026-07-22-067` reverified the finite accepted validator/source inventory without exposing a content defect or concrete uncovered risk. | `test/quality/content_validation_test.dart`; accepted Workshop/Journal/Learning/species/plant/source paths; 152-test Focused/analyze PASS in 20,704 ms; reset-assisted Full PASS in 233,189 ms. | `VERIFY_LOCALLY` | closed | Content and rule confidence | No | Reopen only for changed content/source risks or a concrete uncovered validator gap; accepted breadth is not a quota. |
| DCL-RULE-001 | Current compatibility, unit, and calculation rules have complete requested executable coverage. | `DR-2026-07-22-067` added direct CompatibilityService coverage for temperature, pH, GH, tank size, school size, conflicts, temperament, predation, and severity precedence; numeric proof covers all five Tank Volume shapes and displayed length, volume, hardness, and temperature conversions. No wrong behavior was exposed. | Rule inventory, CompatibilityService, Tank Volume, and Unit Converter tests; 152-test Focused/analyze PASS in 20,704 ms; independent review; reset-assisted Full signing/dependency/lint/tests/analyze/APK PASS in 233,189 ms. | `VERIFY_LOCALLY` | closed | Content and rule confidence | No | Reopen only if governed rules/conversions change or contradictory executable evidence proves wrong behavior. |
| DCL-A11Y-001 | All five ordered phone accessibility clusters are complete. | `DR-2026-07-25-080` adds permanent 2.0x consent and destructive/recovery action-reachability proof, clean independent review, and Full. | `FINISH_MAP.md` Accessibility row; `docs/qa/phone-quality/2026-07-25/dcl-a11y-001-first-run-destructive-recovery-dialogs.md`; backlog `CL-P2-004`. | `VERIFY_LOCALLY` | closed | Phone accessibility and visual polish | No | Reopen only for contradictory current P0/P1 evidence. |
| DCL-VIS-001 | Current phone assets and concrete P0/P1 visual defects are verified across all five clusters. | Cluster 5 screenshots and tests cover consent/dialog reflow; no asset bytes changed. | `docs/qa/phone-quality/2026-07-25/dcl-a11y-001-first-run-destructive-recovery-dialogs.md`; `docs/design/BASELINES.md`. | `VERIFY_LOCALLY` | closed | Phone visual polish | No | Reopen only for contradictory current P0/P1 evidence or a later governed visual change. |
| DCL-VIS-002 | Selective phone visual regression proof is complete for all five clusters. | Current July screenshots, golden harness, permanent 2.0x selectors, and Cluster 5 targeted proof form the bounded baseline set. | `FINISH_MAP.md` Whole-app phone audit row; `docs/design/BASELINES.md`; `test/quality/visual_baseline_manifest_test.dart`; `docs/qa/phone-quality/2026-07-25/dcl-a11y-001-first-run-destructive-recovery-dialogs.md`. | `VERIFY_LOCALLY` | closed | Phone visual polish | No | Keep targeted proof for later changed high-risk surfaces. |
...
| DCL-MOTION-001 | Global haptic-preference enforcement and all five phone-cluster checks are complete. | All product haptics route through the persisted adapter; the source guard permits platform haptics only there. Cluster 5 adds no bypass. | `DR-2026-07-22-068`; `DR-2026-07-25-080`; `FINISH_MAP.md` Motion and haptics row. | `VERIFY_LOCALLY` | closed | Phone motion and visual polish | No | Reopen only for contradictory current P0/P1 evidence. |
| DCL-PERF-001 | Android phone performance has current profile evidence; Tank and scrolling retain emulator dropped-frame percentage misses without an isolated incremental product cause. | `DR-2026-07-25-081` reran exact commit `574b28b9` after user-authorized `danio_api36` recovery. Four latency scenarios and both frame averages pass; raw Tank/scroll percentage counters remain failed. Prior paired Tank idle/feeding and Learn/minimal-scroll traces isolate the same raster floor rather than incremental product work. | `docs/qa/performance/2026-07-25/dcl-perf-001-phone-profile-current-rerun.json`; `docs/qa/performance/2026-07-25/dcl-perf-001-dropped-frame-formal-disposition.md`; prior paired-attribution report; AndroidPrep PASS in 193,098 ms. | `VERIFY_LOCALLY` | accepted | Phone performance | No | Reopen for a current product-attributable trace, ANR, average-time regression, or contradictory phone P0/P1 evidence. Preserve unchanged budgets and honest failed raw counters; physical-device and public-release claims remain outside this local-emulator disposition. |
...
| DCL-RC-001 | Final local Android phone acceptance is complete; every preceding phone row is closed, accepted, or parked. | `DR-2026-07-25-082` recomputed the 96-screen phone accounting, ran final route/render, content/rule, data/privacy, Visual, Full, AndroidPrep, current performance, and serial-pinned Android journey/black-box checks. No unresolved P0/P1 remains. | `docs/qa/2026-07-25-dcl-rc-001-final-local-phone-candidate.md`; product `bc76532e`; debug APK SHA-256 `A63ADF60206AC44E918C6D363E917B9A84632685143C299F176673B915439BFD`. | `VERIFY_LOCALLY` | closed | Final evidence | No | Stop the local phone program. Tablet, store/public release, cloud/accounts, paid/provider-key work, and iOS remain separately parked or external. |
