...
| DCL-A11Y-001 | Phone accessibility remains open across contrast, touch targets, labels, text scaling, reduced motion, and non-colour-only status; cluster 1 is complete and four phone-quality clusters remain. | `DR-2026-07-24-075` binds permanent Tank Detail/Add Log 2.0x tests and final device proof after the prior Tank-root 48 dp fix. | `FINISH_MAP.md` Accessibility row; `docs/qa/phone-quality/2026-07-24/dcl-a11y-001-tank-large-text-visual-bundle.md`; backlog `CL-P2-004`. | `VERIFY_LOCALLY` | open | Phone accessibility and visual polish | No | Each ordered phone cluster is audited with current screenshots and focused widget/accessibility paths; all concrete failures are fixed and the affected tests and Visual checks pass, with no untracked phone cluster remaining. |
| DCL-VIS-001 | Current phone assets and visual defects still need verification across four remaining clusters; cluster 1 is complete. | The active ocean-room/Neon Tetra rights basis and Tank Detail/Add Log clipping are resolved with attributable evidence and current screenshots; the March audit remains historical context only. | `docs/qa/phone-quality/2026-07-24/dcl-a11y-001-tank-large-text-visual-bundle.md`; `docs/qa/screenshots/2026-07-24/dcl-a11y-001-tank-large-text/`; `docs/design/BASELINES.md`. | `VERIFY_LOCALLY` | open | Phone visual polish | No | Current phone screenshots and repo assets are inspected; every concrete weak, mismatched, missing, licensed-source, clipping, or contrast defect is fixed with affected Visual proof, or no-current-gap evidence closes the row. There is no asset-replacement quota. |
| DCL-VIS-002 | Selective visual regression remains open after targeted cluster-1 proof; four phone-quality clusters remain. | Current July phone screenshots, the existing golden harness, and the bound Tank/Add Log 2.0x before/after evidence provide a bounded baseline set. | `FINISH_MAP.md` Whole-app phone audit row; `docs/qa/screenshots/2026-07-24/dcl-a11y-001-tank-large-text/`; `docs/design/BASELINES.md`; `test/quality/visual_baseline_manifest_test.dart`. | `VERIFY_LOCALLY` | open | Phone visual polish | No | The baseline manifest points to current phone evidence, changed high-risk surfaces have targeted screenshot/golden proof where useful, and all current baseline checks pass; no minimum number of new goldens is required. |
...
| DCL-MOTION-001 | Global haptic-preference enforcement and cluster-1 evidence are complete; four phone-quality clusters remain for reduced-motion and quality evidence. | All product haptics route through one persisted preference-aware adapter; the source guard permits platform haptics only inside it; cluster 1 adds no haptic or animation change and reuses the passing disabled-zero-call and reduced-motion checks. | `DR-2026-07-22-068`; `DR-2026-07-24-075`; `FINISH_MAP.md` Motion and haptics row; `docs/qa/phone-quality/2026-07-24/dcl-a11y-001-tank-large-text-visual-bundle.md`. | `VERIFY_LOCALLY` | open | Phone motion and visual polish | No | The global haptic adapter, source guard, zero-call disabled proof, and exact enabled-call proof pass; reduced-motion and the remaining quality checks pass across all five phone-quality clusters. |
