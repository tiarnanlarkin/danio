...
| DCL-AI-001 | Any local persistence of AI output requires explicit user confirmation. Fish ID proof is complete; AI Compatibility retains the known unconfirmed AI-history persistence gap. | `DR-2026-07-21-064` proved Fish ID shows its result before consent, writes nothing on cancel/system back, writes once on confirmation, and keeps the result visible after history failure; Compatibility still writes immediately. | `lib/features/smart/fish_id/fish_id_screen.dart`; `test/widget_tests/fish_id_screen_test.dart`; `lib/widgets/compatibility_checker_widget.dart`; Focused PASS in 16,550 ms; independent review; reset-assisted Full signing/dependency/lint/2,286 tests/analyze/APK PASS in 177,895 ms. | `VERIFY_LOCALLY` | open | Optional AI | No | Fish ID and AI Compatibility each prove dismiss/cancel writes nothing, confirm writes exactly once, and a history-write failure never hides the visible result; every already-gated AI surface is reverified. |
