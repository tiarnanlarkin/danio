...
| Species and plants | `DCL-P1-006` | Implemented | Current 75+ species and 40+ plants include profiles, care actions, wishlist/tank/task handoffs, source trails, and a missing-entry request path; this breadth is accepted for phone completion. | Run the current validators and representative phone-path audit; fix only a concrete data, source, image, handoff, or display defect, or close with no-current-gap evidence. |
| Learning | `DCL-P1-005` | In progress | The current 82 lessons and structured guide paths are accepted breadth; path-load failures are retryable, story exit protects progress, locked cards explain requirements, and path cards open a dedicated sequence view. | Run the current validators and representative phone-path audit; fix only a concrete navigation, lock, practice-link, citation, content, or visual defect, or close with no-current-gap evidence. |
...
| Backup and restore | `DCL-DR-001,DCL-DR-002,DCL-DR-004` | In progress | `DR-2026-07-16-001` maps the current restore matrix and fixes F1 preference error preservation. `DR-2026-07-16-002` fixes F2 share outcomes and completed-ZIP cleanup. `DR-2026-07-16-003` proves cancel/empty picker outcomes return idle without writes and gives pathless/`unknown_path` failures accurate access feedback. Existing evidence also covers validation, tank/child rollback, referenced-photo-only extraction, best-effort restored-photo cleanup, no-tank preference guards, relationship preflight, migration, corruption recovery, and account-keyed cloud backup copy honesty. | Prove `DCL-DR-001-F4` confirmation cancellation next, then close simultaneous tank-import/rollback-failure and mid-extraction-cleanup evidence before selecting `DCL-DR-002`; fix only a concrete gap proven by the current test-first slice. |
...
- Fish ID and AI Compatibility still persist AI-history summaries without a
  confirmation step. Each is a known, separate future single-slice epoch under
  `DCL-AI-001`; do not combine them or treat other AI surfaces as unconfirmed
...
`DCL-DR-001-F3` are locally fixed, while `DCL-DR-001-F4` confirmation
cancellation remains unproven. Begin that exact evidence slice only after this
checkpoint is clean, pushed, and aligned. Do not select `DCL-DR-002` until the
restore matrix has no unexplained current path.
