...
| Learning | In progress | Structured guide coverage exists for all current paths, the placeholder placement CTA is hidden until a real flow exists, path-load failures show retryable errors, story play asks before leaving mid-story progress, and locked story cards explain unlock requirements. | Add richer visuals, practice links, scenarios, citations, and broader interaction variety. |
| Practice | Implemented | Skill Drills and scenario practice cover parameters, diagnosis, compatibility, setup, and emergency decisions, with distinct Learn entry points, SR provider error coverage, and legacy-card recall prompts. | Add persisted tool-result context only where walkthroughs prove it improves flow. |
