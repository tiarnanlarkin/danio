...
| Whole-app phone audit | In progress | Current phone screenshot/XML pass exists under `docs/qa/screenshots/2026-07-04/cl-qa-001-phone-whole-app-map/`, with notes in `docs/qa/whole-app-phone-map-2026-07-04.md`; the Smart/Fish ID bottom-dock overlap and smoke tap false positive are fixed locally with tests, the phone black-box smoke rerun passed, `phone-04b-smart-root-after-dock-fix` recaptures Fish & Plant ID clearing the dock by `47px`, and all 96 `SCREEN_INVENTORY.md` rows now have phone `Pass` or `Gap` accounting. | Capture or route-smoke the 67 phone gap rows, prioritizing normal-user routes before debug-only surfaces. |
| Whole-app tablet audit | In progress | Current 19-surface tablet screenshot/XML pass exists under `docs/qa/screenshots/2026-07-04/cl-qa-002-tablet-whole-app-map/`, with notes in `docs/qa/whole-app-tablet-map-2026-07-04.md`; smoke route assertions/tap handling and lower More hub tablet swipes are hardened locally with tests, the tablet black-box smoke rerun passed, and all 96 `SCREEN_INVENTORY.md` rows now have tablet `Pass` or `Gap` accounting. | Capture or route-smoke the 67 tablet gap rows, prioritizing normal-user routes before debug-only surfaces. |
