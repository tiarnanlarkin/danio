...
`DCL-DR-003-F35` are settled evidence, but `DCL-DR-003` remains open. The
settled range is F1 through F35. Tasks completion compensation is complete in
`DR-2026-07-19-058`; run Equipment Mark Serviced compensation next, then keep
single livestock-add compensation and the Wishlist replay probe in their fixed
separate order. Reinspect the current matrix before closing `DCL-DR-003`; keep
`DCL-DR-004` after it.
