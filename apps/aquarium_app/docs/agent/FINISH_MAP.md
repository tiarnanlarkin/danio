...
`DCL-DR-003-F36` are settled evidence, but `DCL-DR-003` remains open. The
settled range is F1 through F36. Tasks and Equipment Mark Serviced compensation
are complete in `DR-2026-07-19-058` and `DR-2026-07-19-059`; run single
livestock-add compensation next, then keep the Wishlist replay probe separate.
Reinspect the current matrix before closing `DCL-DR-003`; keep
`DCL-DR-004` after it.
