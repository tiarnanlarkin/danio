...
| Motion and haptics | `DCL-MOTION-001` | In progress | `DR-2026-07-22-068` routes all product haptics through one persisted adapter; its source guard, disabled-zero-call, exact enabled-call, duplicate-protection, and existing reduced-motion checks pass. | After the profile harness, run the five phone-quality clusters and fix only proven P0/P1 reduced-motion, haptic, accessibility, clipping, contrast, or asset-rights failures. No new-animation quota applies. |
