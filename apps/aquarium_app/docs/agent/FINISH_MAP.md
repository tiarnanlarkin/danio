...
| Accepted | Current evidence is complete and an explicit bounded limitation is non-blocking for this candidate; raw failed evidence remains honest and defines the reopen condition. |
...
| Visual asset quality | `DCL-VIS-001` | Done | All five ordered phone clusters have current asset, clipping, contrast, and provenance evidence through `DR-2026-07-25-080`. | Reopen only for a later governed visual change or contradictory current P0/P1 evidence. There is no replacement quota. |
| Accessibility | `DCL-A11Y-001` | Done | All five phone-quality clusters have current 2.0x, focused, Visual, and bounded phone evidence. Cluster 5 adds consent plus destructive/data-recovery dialog proof. | Keep regression coverage active; reopen only for contradictory current P0/P1 evidence. |
| Motion and haptics | `DCL-MOTION-001` | Done | Global haptic enforcement and all five ordered phone-quality cluster checks are complete. Cluster 5 adds no bypass. | Keep adapter/source guards active; no new-animation quota applies. |
| Performance | `DCL-PERF-001` | Accepted | Exact commit `574b28b9` passes cold start, warm resume, tabs, local-image paint, Tank average, and scrolling average on `danio_api36`. Tank and scrolling raw dropped-frame percentages remain failed; paired idle/feeding and Learn/minimal traces isolate no incremental product P1, so `DR-2026-07-25-081` accepts only the emulator percentage floor without relaxing targets or relabelling the report. | Reopen for a current product-attributable trace, ANR, average-time regression, or contradictory P0/P1 evidence. Physical phone, tablet, store, and public release remain outside this accepted local candidate boundary. |
...
| Whole-app phone audit | `DCL-RC-001` | Done | All 96 `SCREEN_INVENTORY.md` rows retain phone Pass accounting. `DR-2026-07-25-082` adds final route/render, content/rule, data/privacy, Visual, Full, AndroidPrep, current performance, and serial-pinned Android journey/black-box evidence at product `bc76532e`, with no unresolved P0/P1. | Reopen only for contradictory current phone evidence or a separately authorized release phase. |
...
| Visual regression | `DCL-VIS-002` | Done | Current July phone screenshots, golden tests, and the visual baseline manifest cover all five phone clusters with targeted large-text proof. | Keep targeted proof for later changed high-risk surfaces. No new-golden quota applies. |
