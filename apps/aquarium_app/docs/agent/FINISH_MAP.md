...
| Whole-app phone audit | In progress | Current phone screenshot/XML pass exists under `docs/qa/screenshots/2026-07-04/cl-qa-001-phone-whole-app-map/`, with notes in `docs/qa/whole-app-phone-map-2026-07-04.md`; the Smart/Fish ID bottom-dock overlap and smoke tap false positive are fixed locally with tests, the phone black-box smoke rerun passed, and `phone-04b-smart-root-after-dock-fix` recaptures Fish & Plant ID clearing the dock by `47px`. | Finish full CL-QA-001 inventory coverage for every remaining `SCREEN_INVENTORY.md` surface. |
