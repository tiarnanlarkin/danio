...
fail after a fresh-ID tank is durable. It is locally fixed in
`DR-2026-07-18-046` under marker
`danio-dcl-dr-003-tank-create-rollback-uncertainty-proof-2026-07-18/1`:
dual task-save/tank-delete failure now preserves both causes and the possibly
durable tank ID, refreshes provider authority, warns without Retry, and locks
...
compensation retains safe Retry.
...
a time and records the originals for reverse compensation. `DR-2026-07-19-052`
locally fixes F32 under marker
`danio-dcl-dr-003-livestock-bulk-move-rollback-uncertainty-proof-2026-07-19/1`.
`LivestockBulkMoveCompensationException` preserves the initiating target-save
error, every keyed rollback failure, uncertain selected IDs, and source/target
tank context. The uncertain path invalidates source and target tank and
livestock authority, clears stale selection, and warns that persistence may be
split without success, Retry, or try-again feedback. `bulk move preserves
initiating and rollback failures when compensation is uncertain`, `failed
bulk-move rollback reports partial-move uncertainty without unsafe retry`, and
`rolls back earlier moves when a later save fails` are GREEN. All 58 affected
provider/widget tests and analyze passed through the explicit Focused gate.
Independent read-only review found no findings; the reset-assisted Full gate
passed in 245,604 ms after the documented failed recovery attempts.
DCL-DR-003 remains open. Task completion, equipment edit/service,
...
`danio-dcl-dr-003-next-finding-triage-2026-07-19/7`. `DR-2026-07-19-054`
locally fixes it under marker
`danio-dcl-dr-003-inventory-use-rollback-uncertainty-proof-2026-07-19/1`.
`InventoryUseCompensationException` preserves the initiating profile-effect
error/stack, every inventory-restore failure/stack, and the affected item
ID/name/type. The uncertain path refreshes inventory and profile authority,
does not falsely restore in-memory inventory, warns that Streak Freeze may have
been consumed without its effect, and locks stale use callbacks without success
or Retry. Initial inventory-save failure and successful compensation keep their
definitive behavior. Named evidence is `useItem preserves effect and rollback failures when inventory restore is uncertain`, `useItem restores inventory after profile effect failure when compensation succeeds`, and `failed consumable rollback reports lost-item uncertainty without unsafe retry`. The final 62-test Focused gate and analyze passed; independent settled-diff review found no findings after the stale-callback proof was strengthened; the unchanged final Full gate passed after the two recorded recovery attempts.
DCL-DR-003 remains open. Equipment Mark Serviced compensation, task/equipment
...
completed task before its completion log. If `task completion log failed` and
restoring the original task then throws `task rollback failed`, the rollback
cause is logged and discarded, the task remains completed, and the UI says
`Try again`. Existing `failed completion log write rolls back task completion`
proves only successful restoration. Temporary RED `failed tank-detail task
rollback reports uncertain completion without unsafe retry` repeated the
still-active action after that uncertain first attempt; the completion count
reached two while both attempts omitted the rollback cause. The probe was
removed. F34 remains unimplemented under
`danio-dcl-dr-003-tank-detail-task-completion-rollback-uncertainty-proof-2026-07-19/1`.
DCL-DR-003 remains open. The parallel Tasks screen rollback boundary,
Equipment Mark Serviced, Wishlist replay, single-livestock-add, and all other
unexplained boundaries remain separate and unranked.

Implementation epoch `DR-2026-07-19-056` locally fixed `DCL-DR-003-F34`
under `danio-dcl-dr-003-tank-detail-task-completion-rollback-uncertainty-proof-2026-07-19/1`.
`TankDetailTaskCompletionCompensationException` preserves both causes with task/tank context
when completion-log persistence and task restoration both fail. One durable
completion remains; task, equipment, and log authority refreshes; the warning
offers no success, Retry, or `Try again`; and visible
and stale completion callbacks are locked. `failed tank-detail task rollback
...
Implementation epoch `DR-2026-07-19-060` locally fixed only
`DCL-DR-003-F37` under
`danio-dcl-dr-003-single-livestock-add-rollback-uncertainty-proof-2026-07-19/1`.
`LivestockAddCompensationException` preserves the initiating error and stack,
all rollback errors and stacks, and the tank, livestock, and activity-log
identifiers when activity-log persistence and both cleanup writes cannot settle
safely. The uncertain path reloads tank, livestock, recent-log, and full-log
authority; uncertain feedback has no success or unsafe Retry; and in-flight,
visible, dismissed, and
stale callbacks cannot repeat the write. `failed single-add log and deletion
rollback preserves uncertain id and blocks repeated submit`, `in-flight
single-add ignores a repeated stale callback`, and `stale single-add Retry is
ignored after sheet dismissal` are GREEN alongside clean compensation. The
final 30-test Focused gate plus analyze, independent settled-diff review, and
reset-assisted Full with 2,276 tests/lint/analyze/APK pass; final Docs passes 24
...
| --- | --- | --- |
| Tank create plus default tasks | `creates a new tank and persists it to storage`; `creates default tasks along with the tank`; `rolls back tank and partial default tasks if default task save fails`; `createTank preserves task-save and rollback failures when cleanup is uncertain`; `successful guided creation closes the wizard`; `failed tank-create rollback reports uncertainty and blocks duplicate retry`; `stale tank-create retry cannot bypass uncertain persistence lock`; `clean tank-create compensation retains safe Retry` | `DCL-DR-003-F29` locally fixed: failed tank deletion produces a typed dual-cause error with the possibly durable ID, invalidates tank/task authority, clears Retry, warns about the possibly existing tank and incomplete default tasks, and locks visible and stale submission paths. Definitive compensation retains safe generic Retry. |
...
| Livestock create/edit | `adding livestock shows success feedback and readable timeline log`; `failed add-log save rolls back new livestock`; `failed single-add log and deletion rollback preserves uncertain id and blocks repeated submit`; `in-flight single-add ignores a repeated stale callback`; `stale single-add Retry is ignored after sheet dismissal`; `profile activity failure after livestock add does not report add failure`; `stale livestock edit ids are not recreated by save`; `missing tank ids do not create orphan livestock` | `DCL-DR-003-F37` is locally fixed in `DR-2026-07-19-060`: `LivestockAddCompensationException` preserves the initiating and rollback failures/stacks plus tank/livestock/log IDs, reloads tank/livestock/recent-log/full-log authority, warns without success or unsafe Retry, and locks visible, in-flight, dismissed, and stale callbacks. Definitive compensation retains safe Retry. |
...
| Cost delete/clear/undo | `clearing all expenses shows undo and restores saved expenses`; `undo restore failure shows local feedback without throwing`; `single expense undo restore failure keeps expense deleted with feedback`; `single expense delete failure keeps expense visible with feedback`; `clear false save result shows feedback and keeps expenses active`; RED/GREEN `rapid expense dismissals delete both expenses without stale-index failure` | `DCL-DR-003-F22` locally fixed: each rendered callback captures an expense ID and resolves its current index immediately before mutation, so rapid dismissals delete both expenses without a stale-index exception. Currency failure already rolls visible state back with error feedback but lacks exact evidence. |
...
| Inventory | Migration false-save, `useItem surfaces inventory save failures before applying profile effect`, `failed item use shows retry feedback and keeps the item visible`, purchase refund, duplicate permanent, reset failure, and RED/GREEN `expired item cleanup failure shows feedback without changing inventory` | Refund failure is covered by F24 and startup cleanup by fixed F25. `DCL-DR-003-F33` is locally fixed in `DR-2026-07-19-054`: `InventoryUseCompensationException` preserves both sides of uncertain restoration with item context, refreshes inventory and profile authority, and `failed consumable rollback reports lost-item uncertainty without unsafe retry`. `useItem restores inventory after profile effect failure when compensation succeeds` preserves definitive compensation. Missing catalog handling remains excluded; DCL-DR-003 remains open. |
...
19. `DCL-DR-003-F19` - Local Shop Edit rejects a shop deleted behind its open
    sheet: fixed and focused GREEN in `DR-2026-07-16-033` under marker
    `danio-dcl-dr-003-local-shop-edit-stale-id-proof-2026-07-16/1`.
20. `DCL-DR-003-F20` - Local Shop Remove rejects a shop deleted behind its
...
22. `DCL-DR-003-F22` - rapid Cost Tracker dismissals resolve each expense by
    stable ID instead of reusing stale list indices: fixed and focused GREEN in
    `DR-2026-07-16-036` under marker
    `danio-dcl-dr-003-cost-delete-stale-index-proof-2026-07-16/1`.
23. `DCL-DR-003-F23` - Review Completion no longer turns a durable session count
    and streak into false failure through a redundant late mirror save: fixed
    and focused GREEN in `DR-2026-07-16-037` under marker
    `danio-dcl-dr-003-review-completion-redundant-save-proof-2026-07-16/1`.
24. `DCL-DR-003-F24` - a failed Gem Shop inventory save plus failed refund now
    preserves both errors for diagnosis and reports the uncertain gem balance
...
27. `DCL-DR-003-F27` - `TasksScreen` completion (not Tank Detail) now catches
    and logs a post-commit profile/XP persistence failure after durable task,
    log, and any linked equipment/service writes. Provider invalidations still
    run, durable primary writes are not retried, and feedback warns that
    progress could not update without offering Retry. Ranked in
    `DR-2026-07-18-041` and fixed and focused GREEN in `DR-2026-07-18-042` under marker
...
29. `DCL-DR-003-F29` - new-tank creation now distinguishes definitive cleanup
    from uncertain cleanup. `TankCreateCompensationException` retains the
    initiating default-task save error, failed tank-delete compensation, and
    possibly durable tank ID; tank and task providers are invalidated before it
    is exposed. `CreateTankScreen` clears active/queued Retry feedback, warns
    that the tank may already exist with incomplete default tasks, disables both
    Create buttons, and guards stale callbacks. Clean compensation preserves
    generic Retry. Ranked in `DR-2026-07-18-045` under marker
    `danio-dcl-dr-003-next-finding-triage-2026-07-18/3`; fixed and focused GREEN
    in `DR-2026-07-18-046` under marker
    `danio-dcl-dr-003-tank-create-rollback-uncertainty-proof-2026-07-18/1`.

    Direct RED proved the provider rethrew only the task-save `StateError`, the
    ordinary Retry created a second UUID, and a stale captured Retry created a
    third. `createTank preserves task-save and rollback failures when cleanup is
    uncertain`, `failed tank-create rollback reports uncertainty and blocks
...
32. `DCL-DR-003-F32` - ranked only in `DR-2026-07-19-051` under marker
    `danio-dcl-dr-003-next-finding-triage-2026-07-18/6` and locally fixed in
    `DR-2026-07-19-052` under marker
    `danio-dcl-dr-003-livestock-bulk-move-rollback-uncertainty-proof-2026-07-19/1`.
    The provider and widget REDs proved a split persisted result, discarded
    rollback cause, and unsafe try-again feedback. The typed
    `LivestockBulkMoveCompensationException` now preserves the initiating error,
    every rollback failure, uncertain livestock IDs, and source/target context.
    Source and target tank and livestock authority refreshes after uncertainty;
    the screen exits stale selection and warns without success or Retry. `bulk
    move preserves initiating and rollback failures when compensation is
    uncertain`, `failed bulk-move rollback reports partial-move uncertainty
    without unsafe retry`, and the definitive-compensation test `rolls back
...
33. `DCL-DR-003-F33` - ranked only in `DR-2026-07-19-053` under marker
    `danio-dcl-dr-003-next-finding-triage-2026-07-19/7` and locally fixed in
    `DR-2026-07-19-054` under marker
    `danio-dcl-dr-003-inventory-use-rollback-uncertainty-proof-2026-07-19/1`.
    The provider/UI REDs proved durable consumable loss, absent profile effect,
    discarded restore failure, false retry, and unsafe try-again guidance.
    `InventoryUseCompensationException` now preserves the effect error/stack,
    every restore failure/stack, and item ID/name/type, then refreshes inventory
    and profile authority without falsely restoring memory. The screen warns
    that the item may have been consumed without benefit and locks visible plus
    stale use paths without success or Retry. `useItem preserves effect and
    rollback failures when inventory restore is uncertain`, `useItem restores
    inventory after profile effect failure when compensation succeeds`, and
    `failed consumable rollback reports lost-item uncertainty without unsafe retry`
    are GREEN. The final 62-test Focused gate and analyze passed; independent
    settled-diff review found no findings after the stale-callback proof was
    strengthened; the unchanged final Full gate passed after the two recorded
    recovery attempts. DCL-DR-003 remains open; no second finding is ranked here.
34. `DCL-DR-003-F34` - ranked only in `DR-2026-07-19-055` under marker
    `danio-dcl-dr-003-next-finding-triage-2026-07-19/8` and locally fixed in
    `DR-2026-07-19-056` under
    `danio-dcl-dr-003-tank-detail-task-completion-rollback-uncertainty-proof-2026-07-19/1`.
    `TankDetailTaskCompletionCompensationException` preserves both causes with
    task/tank context. Dual failure leaves one durable completion, refreshes
    task, equipment, and log authority, warns with no success, Retry, or
    `Try again`, and locks visible and stale completion callbacks. `failed
    tank-detail task rollback reports uncertain completion without unsafe retry`
...
37. `DCL-DR-003-F37` - locally fixed only in `DR-2026-07-19-060` under
    `danio-dcl-dr-003-single-livestock-add-rollback-uncertainty-proof-2026-07-19/1`.
    `LivestockAddCompensationException` preserves the initiating error and stack,
    all rollback errors and stacks, and the tank, livestock, and activity-log
    identifiers. When activity-log persistence and log/livestock cleanup fail,
    the sheet reloads tank/livestock/recent-log/full-log authority, warns without
    success or unsafe Retry, and locks in-flight, visible, dismissed, and stale
