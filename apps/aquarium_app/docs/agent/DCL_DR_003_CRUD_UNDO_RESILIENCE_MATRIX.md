...
Current epoch: `DR-2026-07-16-037`

## Decision

The fresh current-source inventory disproved a no-current-gap close. The first
twenty-two bounded fixes prevent Today Board, Home main-Tank, Livestock quick Feed,
...
commits authoritative, bind Cost Tracker deletion to stable expense IDs, prevent a redundant Review completion mirror from creating false failure, and make lesson progress precede rewards with a real
retry after failure.
...
| Gems | Grant/refund failure tests, add/spend cumulative rollback tests, and `reset surfaces failed local removals before reporting reset success` | Direct writers are covered. `DCL-DR-003-F24` is selected because a failed Gem Shop inventory save followed by a failed refund can leave spent gems durable while generic Retry invites a second spend. |
...
24. `DCL-DR-003-F24` - a failed Gem Shop inventory save plus failed refund must
    report the uncertain gem balance without unsafe immediate Retry. Next marker:
    `danio-dcl-dr-003-gem-purchase-refund-failure-feedback-2026-07-16/1`.
25. The removal-log relationship finding is deferred to `DCL-DR-004`; fixing
