# Danio Slice Contract: DS-2026-07-04-016

## Slice

- ID: `DS-2026-07-04-016`
- Title: Tank Journal manual entries must not create orphan logs
...
- Files/modules explicitly out of scope: Add Log, Tank Detail quick feeding,
  Today Board, home quick actions, backup/restore, Android devices,
  screenshots, and broader migration/app-kill coverage.

## Product Goal

- User-visible outcome: if Tank Journal remains open after its durable parent
  tank was deleted, saving a manual journal entry shows existing retry feedback
  and does not create an orphan local log.
- Complete-local requirement this advances: tank-scoped journal writes must
  respect durable parent tank records before writing local data.
...
- Fresh session recommended: No; this is a narrow continuation after a fresh
  handoff and clean, origin-aligned checkout.
- Repo context checked: `AGENTS.md`, `WORKFLOW_CHARTER.md`,
  ...
  product backlog/current audit, Journal source/tests, and recent
...
- Decision-changing research notes: `JournalScreen._addJournalEntry()` creates
  a `LogEntry` and calls `storage.saveLog()` directly. Existing coverage only
  simulates thrown save failures, so a stale open journal can create a log when
  `storage.getTank(tankId)` would return `null`.

## Design And Visual Target

- Current screenshot/golden/mockup/existing surface: existing Tank Journal entry
  bottom sheet and field-level retry feedback.
- Phone expectation: no layout change.
- Tablet expectation: no layout change.
- Accessibility expectation: existing field error text remains the announced
  feedback path.
...
- Local data touched: Tank Journal manual observation `LogEntry` saves.
- Failure states to test: the Journal route remains mounted with a stale
  `tankId`, but `storage.getTank(tankId)` returns `null` before the log save.
- Rollback or retry behavior: no log is saved, the sheet remains open, and the
  existing retry feedback is shown.
- No-fake-feature/product-honesty check: Tank Journal cannot report a manual
  entry as saved when no durable parent tank exists.

## Done Criteria

The slice is done only when:

- the focused stale-parent Tank Journal test fails before the production
  change;
- the named test and full Journal widget test file pass after the change;
...
- Verification summary: RED named widget test failed because an orphan
  ...
  Journal widget tests passed; targeted analysis passed; `Full` local quality
