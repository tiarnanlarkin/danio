...
black-box smoke rerun passed. It does not close CL-QA-001 because the full
`SCREEN_INVENTORY.md` surface set still needs manual or scripted coverage.
...
| Direct QA deep-link capture | Pass for captured set | 19 `.png` files and 19 paired `.xml` files were saved; every XML contains a `<hierarchy>` root. |
| Narrow crash signature scan | Pass | No matches for `FATAL EXCEPTION`, `AndroidRuntime: FATAL`, `E/flutter`, `RenderFlex overflowed`, or `ANR in` in `phone-logcat-tail.txt`. |
...
| --- | --- | --- | --- | --- |
| CL-QA-001-F1 | P1 | Phone Smart root placed the `Fish & Plant ID` card partly behind the bottom dock. The smoke helper tapped the center of the card's semantics bounds, which landed on the dock/Tank tab and returned to Tank instead of opening the optional-AI setup path. | `phone-04-smart-root.png`; `phone-smoke-failure-smart-overlap.png`; `phone-smoke-failure-smart-overlap.xml` | Resolved locally by nudging the locked Smart cards above the persistent dock and rerunning the phone black-box smoke successfully. Recapture Smart root in the full CL-QA-001 inventory pass. |
...
| Smart root | `screenshots/2026-07-04/cl-qa-001-phone-whole-app-map/phone-04-smart-root.png` / `.xml` | Fail | Smart root loads, but lower optional-AI cards are partly covered by the dock at phone height. |
...
- Smart root should be recaptured after the local dock fix before CL-QA-001 is
  closed.
