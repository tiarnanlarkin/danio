...
| CL-P1-006 | Guided tools | Convert calculators into guided workflows with prefill, explanation, warnings, save/apply, and confirmation. | In progress; CL-P1-006A turns Water Change into the first guided tool workflow with tank-volume prefill from Workshop, result explanation, safety guidance, and a prefilled water-change journal handoff. CL-P1-006B turns Tank Volume into a guided apply workflow that saves calculated litres back to the selected local tank profile. Remaining: apply the guided pattern across the other calculators/tools and add broader tool-result timeline/task handoffs. |
...
Current verification note: as of the Tank Volume guided workflow slice on
2026-06-13, `flutter test` passes 1501 tests and `flutter analyze` is clean,
