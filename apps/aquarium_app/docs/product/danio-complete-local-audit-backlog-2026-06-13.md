...
| CL-QA-005 | Content validation | Add content linting for units, spelling style, missing warnings, missing sources, duplicate IDs, bad ranges, and locked emergency content. | In progress; CL-QA-005A adds a learning-content guard for UK-style litre/litres volume spelling and fixes the current liter/liters lesson copy drift. CL-QA-005B adds a metric-context guard so gallon references in learning copy include litre equivalents, and updates current tank-size and treatment-dose examples. CL-QA-005C adds the same Celsius-context guard for Fahrenheit references. CL-QA-005D adds warning-section coverage for medical/emergency lessons and updates Fish Health prevention, ich, fin rot, and parasite lessons with explicit safety boundaries. CL-QA-005E blocks overconfident or product-endorsing care claims and rewrites current dechlorinator, testing-kit, water-change, and nerite-snail copy to be neutral and label-based. Remaining: source gaps, duplicate IDs, bad ranges, and locked-content checks. |
