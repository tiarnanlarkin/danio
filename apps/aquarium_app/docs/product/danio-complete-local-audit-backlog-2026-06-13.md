...
- CL-P1-009FW makes achievement provider resets reject failed local
  `achievement_progress` removals before clearing visible progress.
- CL-P1-009FX makes lesson-completion achievement checks use the persisted
  completed-lesson list and persisted perfect-score count instead of adding
  inferred extra counts after the profile save.
- CL-P1-009FY makes Wishlist purchase reject missing local item IDs before
  saving or reporting success, preventing stale purchase actions from adding
  budget spend for an item that no longer exists.
- CL-P1-009FZ makes tank edits reject missing local tank IDs before saving,
  preventing stale edit screens from recreating deleted or absent tanks.
- CL-P1-009GA makes livestock edits reject missing local livestock IDs before
  saving, preventing stale edit sheets from recreating deleted livestock.
- CL-P1-009GB makes Add Log edits reject missing local log IDs before saving,
  preventing stale edit forms from recreating deleted logs.
- CL-P1-009GC makes Task edits reject missing local task IDs before saving,
  preventing stale edit sheets from recreating deleted tasks.
- CL-P1-009GD makes Equipment edits reject missing local equipment IDs before
  saving, preventing stale edit sheets from recreating deleted equipment.
- CL-P1-009GE makes Add Log saves reject missing parent tank IDs before
  saving, preventing orphan local logs after tank deletion.
- CL-P1-009GF makes Livestock saves reject missing parent tank IDs before
  saving, preventing orphan livestock and timeline logs after tank deletion.
- CL-P1-009GG makes Task saves reject missing parent tank IDs before saving,
  preventing orphan local tasks after tank deletion.
- CL-P1-009GH makes Equipment saves reject missing parent tank IDs before
  saving, preventing orphan equipment and maintenance tasks after tank deletion.
...
- CL-P1-009GN makes Ask Danio ask before saving typed-question summaries to
  local Recent AI Activity, while still showing the answer immediately.
- CL-P1-009GO makes Cycling Assistant reminder create actions reject missing parent
  tank IDs before saving, preventing orphan local tasks after tank deletion.
- CL-P1-009GP makes bulk Livestock add reject missing parent tank IDs before
  saving, preventing orphan livestock and timeline logs after tank deletion.
- CL-P1-009GQ makes Symptom Triage confirmed journal saves reject missing parent
  tank IDs before saving, preventing orphan local AI diagnosis logs and history
  after tank deletion.
- CL-P1-009GR makes Species detail care-task actions reject missing parent tank
  IDs before saving, preventing orphan local weekly care tasks after tank
  deletion.
- CL-P1-009GS makes Tank Journal manual-entry saves reject missing parent tank
...
- CL-P1-009GW makes direct tank-scoped backup imports reject child entries
...
- CL-P1-009GZ makes direct tank-scoped backup imports reject duplicate backup
  tank IDs before saving imported tanks, preventing duplicate backup tanks from
  collapsing relationship mapping onto one regenerated local tank ID while the
  service reports success.
- CL-P1-009HA makes direct tank-scoped backup imports reject task/log
  relationship IDs whose backup target records belong to a different backup
  tank from the source task/log, preventing cross-tank `relatedEquipmentId`,
  `relatedLivestockId`, or `relatedTaskId` links from being preserved while the
  service reports success.
...
- CL-P1-009HE makes direct tank-scoped backup imports reject malformed
...
- CL-P1-009HG moves direct-import child `tankId` rejection into pre-save
  validation, preventing any imported tank save from being attempted when a
  livestock, equipment, task, or log row references a tank absent from the
  backup.
...
| ID | Area | Work | Acceptance |
| --- | --- | --- | --- |
| CL-P3-001 | Providers | Add provider model: OpenAI, Anthropic, Gemini, OpenRouter, Mistral, and recommended default. | In progress; CL-P3-001A makes Optional AI setup provider-aware by naming OpenAI as the current recommended BYO key provider, listing Anthropic, Google Gemini, OpenRouter, and Mistral as unavailable local key paths in this version, and fixing tall dialog overflow. CL-P3-001B makes build-time `OPENAI_API_KEY` a local-development-only fallback that is ignored in release builds while preserving user-supplied BYO keys and proxy routing. Remaining: implement real non-OpenAI provider connectors before enabling those key paths. |
| CL-P3-002 | Confirm writes | Require confirmation before AI changes tank data, tasks, journal, reminders, or care plans. | In progress; CL-P3-002A makes Symptom Triage ask for confirmation before saving an AI-generated diagnosis to the tank journal, and focused widget coverage verifies canceling the dialog writes no log. CL-P3-002B makes Weekly Plan ask before caching an AI-generated care plan, and focused widget coverage verifies canceling the dialog leaves `weekly_plan_cache` empty. CL-P3-002C moves Symptom Triage AI history writes behind the confirmed journal-save path, and focused widget coverage verifies canceling the dialog leaves `ai_interaction_history` empty. CL-P3-002D makes Ask Danio ask before saving typed-question summaries to Recent AI Activity, and focused widget coverage verifies canceling leaves `ai_interaction_history` empty while confirming saves one `ask_danio` entry. Remaining: confirm-before-write coverage for AI changes to tank data, tasks, and reminders. |
...
| ID | Area | Work | Acceptance |
| --- | --- | --- | --- |
| CL-QA-001 | Screen audit | Re-run a whole-app phone screenshot and XML audit against the complete-local bar. | Locally verified; 2026-07-04 pass captured current phone screenshot/XML evidence under `docs/qa/whole-app-phone-map-2026-07-04.md`. Smart/Fish ID bottom-dock overlap and smoke tap targeting are fixed locally, the phone black-box smoke rerun passed, after-fix Smart root recapture shows Fish & Plant ID clearing the dock by `47px`, QA-006 added 39 standalone deep-link captures, QA-007 added 24 fixed-build seeded tank/learning/story/cycling captures and fixed the Livestock skeleton duplicate-Hero regression, QA-008 added 16 first-run/onboarding/debug captures, and all 96 `SCREEN_INVENTORY.md` rows now have phone `Pass` accounting with 96 passes and 0 current gaps. Recheck when app surfaces change or before release signoff. |
| CL-QA-002 | Tablet audit | Add tablet emulator/screenshot audit. | Locally verified; 2026-07-04 pass captured current tablet screenshot/XML evidence under `docs/qa/whole-app-tablet-map-2026-07-04.md`. Tablet smoke route assertions/taps and lower More hub swipes are hardened locally, the tablet black-box smoke rerun passed, QA-006 added 39 standalone deep-link captures, QA-007 added 24 fixed-build seeded tank/learning/story/cycling captures and fixed the Livestock skeleton duplicate-Hero regression, QA-008 added 16 first-run/onboarding/debug captures, and all 96 `SCREEN_INVENTORY.md` rows now have tablet `Pass` accounting with 96 passes and 0 current gaps. Recheck when app surfaces change or before release signoff. |
...
| CL-QA-005 | Content validation | Add content linting for units, spelling style, missing warnings, missing sources, duplicate IDs, bad ranges, and locked emergency content. | In progress; CL-QA-005A adds a learning-content guard for UK-style litre/litres volume spelling and fixes the current liter/liters lesson copy drift. CL-QA-005B adds a metric-context guard so gallon references in learning copy include litre equivalents, and updates current tank-size and treatment-dose examples. CL-QA-005C adds the same Celsius-context guard for Fahrenheit references. CL-QA-005D adds warning-section coverage for medical/emergency lessons and updates Fish Health prevention, ich, fin rot, and parasite lessons with explicit safety boundaries. CL-QA-005E blocks overconfident or product-endorsing care claims and rewrites current dechlorinator, testing-kit, water-change, and nerite-snail copy to be neutral and label-based. CL-QA-005F extends that guard to brand-specific emergency-product certainty claims and rewrites current emergency conditioner, kit, water-change, and parasite-treatment copy to use neutral label-following guidance. CL-QA-005G blocks remaining branded conditioner/test-kit learning copy and rewrites current ammonia emergency, test-kit, and power-outage guidance to generic label-following product categories. CL-QA-005H adds source-density, learning graph, lesson structure, quiz answer-index, reward, and duration range validation. Remaining: broader locked-content checks and any source/content-risk gaps found during future audits. |
| CL-QA-006 | Data resilience | Test create/edit/delete tank, log, task, livestock, lesson completion, backup/restore, migration, and app-kill flush paths. | In progress; Weekly Plan cache clear now waits for durable local cache removal before hiding the visible plan, and failed removals keep the plan visible. Achievement provider resets now reject failed local progress removals before clearing visible progress. Lesson-completion achievement checks now use persisted completed-lesson and perfect-score profile state. Wishlist purchase now rejects stale/missing local item IDs before success or budget spend. Tank edits now reject stale/missing local tank IDs before saving. Log edits now reject stale/missing local log IDs before saving. Livestock edits now reject stale/missing local livestock IDs before saving. Task edits now reject stale/missing local task IDs before saving. Equipment edits now reject stale/missing local equipment IDs before saving. Add Log, Livestock, Task, Equipment, Cycling Assistant reminder, bulk Livestock, Symptom Triage journal, Species detail care-task, Tank Journal manual-entry, and Tank Detail quick-feeding child saves now reject missing parent tank IDs before saving. Bulk tank delete expiry failures now restore visible tank state and show retry feedback. Direct tank-scoped backup imports now reject child rows whose tank IDs are not imported, duplicate tank/child backup IDs, missing relationship targets, malformed relationship ID types, and cross-tank relationship targets before reporting success. Backup Restore now skips archive-only photo files that are not referenced by validated backup data. Remaining: broader create/edit/delete, restore, migration, and app-kill flush coverage. |
