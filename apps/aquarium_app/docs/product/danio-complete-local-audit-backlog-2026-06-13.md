...
| CL-QA-005 | Content validation | Add content linting for units, spelling style, missing warnings, missing sources, duplicate IDs, bad ranges, and locked emergency content. | In progress; CL-QA-005A adds a learning-content guard for UK-style litre/litres volume spelling and fixes the current liter/liters lesson copy drift. CL-QA-005B adds a metric-context guard so gallon references in learning copy include litre equivalents, and updates current tank-size and treatment-dose examples. CL-QA-005C adds the same Celsius-context guard for Fahrenheit references. Remaining: warning coverage, source gaps, duplicate IDs, bad ranges, and locked-content checks. |
