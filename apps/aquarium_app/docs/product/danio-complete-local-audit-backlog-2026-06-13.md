...
| CL-P1-003 | Species pages | Upgrade species/plant pages into beautiful actionable guides. | In progress; CL-P1-003A adds a simple missing-species request path from Fish Database empty search. CL-P1-003B adds data-derived Care Actions to fish species detail sheets. CL-P1-003C hands species detail pages into a prefilled Stocking Calculator at the minimum group size. Remaining: richer care pages, source trail, saved wishlist/add-to-tank actions, reminders, common problems, and plant-page parity. |
...
Current verification note: as of the species stocking handoff slice on
2026-06-13, `flutter test` passes 1447 tests and `flutter analyze` is clean,
