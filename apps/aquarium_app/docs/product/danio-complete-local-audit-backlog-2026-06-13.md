...
| ID | Area | Work | Acceptance |
| --- | --- | --- | --- |
| CL-QA-001 | Screen audit | Re-run a whole-app phone screenshot and XML audit against the complete-local bar. | In progress; 2026-07-04 pass captured current phone screenshot/XML evidence under `docs/qa/whole-app-phone-map-2026-07-04.md`. Smart/Fish ID bottom-dock overlap and smoke tap targeting are fixed locally, the phone black-box smoke rerun passed, after-fix Smart root recapture shows Fish & Plant ID clearing the dock by `47px`, QA-006 added 39 standalone deep-link captures, QA-007 added 24 fixed-build seeded tank/learning/story/cycling captures and fixed the Livestock skeleton duplicate-Hero regression, and all 96 `SCREEN_INVENTORY.md` rows now have phone `Pass` or `Gap` accounting with 80 passes and 16 gaps. Remaining: capture or route-smoke the 16 phone onboarding/debug-seed gap rows before treating the phone whole-app audit as current visual evidence. |
| CL-QA-002 | Tablet audit | Add tablet emulator/screenshot audit. | In progress; 2026-07-04 pass captured current tablet screenshot/XML evidence under `docs/qa/whole-app-tablet-map-2026-07-04.md`. Tablet smoke route assertions/taps and lower More hub swipes are hardened locally, the tablet black-box smoke rerun passed, QA-006 added 39 standalone deep-link captures, QA-007 added 24 fixed-build seeded tank/learning/story/cycling captures and fixed the Livestock skeleton duplicate-Hero regression, and all 96 `SCREEN_INVENTORY.md` rows now have tablet `Pass` or `Gap` accounting with 80 passes and 16 gaps. Remaining: capture or route-smoke the 16 tablet onboarding/debug-seed gap rows before treating the tablet whole-app audit as current visual evidence. |
...
| CL-QA-006 | Data resilience | Test create/edit/delete tank, log, task, livestock, lesson completion, backup/restore, migration, and app-kill flush paths. | In progress; Weekly Plan cache clear now waits for durable local cache removal before hiding the visible plan, and failed removals keep the plan visible. Achievement provider resets now reject failed local progress removals before clearing visible progress. Lesson-completion achievement checks now use persisted completed-lesson and perfect-score profile state. Remaining: broader create/edit/delete, restore, migration, and app-kill flush coverage. |
